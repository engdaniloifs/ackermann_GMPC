...
from GMPC_Tracking_Control_main.utils.enum_class import TrajType, ControllerType, LiniearizationType
    ...
    def solve(self, current_state, t):
        ...
        for i in range(N):
            ...
            cost += ca.mtimes([x_var[:, i].T, Q, x_var[:, i]]) + ca.mtimes(
                [(u_var[:, i] - self.vel_cmd_to_ackerman_input(u_d)).T, R, (u_var[:, i] - self.vel_cmd_to_ackerman_input(u_d) )])
